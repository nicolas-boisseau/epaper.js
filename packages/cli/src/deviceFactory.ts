...
async function getRpi9In7(orientation?: Orientation, colorMode?: ColorMode): Promise<DisplayDevice> {
    try {
        const { Rpi9In7 } = await import('@epaperjs/rpi-9in7');
        return new Rpi9In7(orientation, colorMode);
    } catch (e) {
        throw new Error('Failed to import @epaperjs/rpi-9in7, make sure it is installed');
    }
}
